...
import { Circle, Eye, EyeOff, RefreshCw, Save } from 'lucide-react';
import { saveAs } from 'file-saver';

import { createAutoencoderModel, processImage, FilterType } from './tf/autoencoderModel';
...
import CircularAreaSelector, { CircleArea } from './components/CircularAreaSelector';
...
function App() {
  ...
  useEffect(() => {
    async function loadModel() {
      try {
        ...
        setError('Failed to initialize TensorFlow.js model. Please try refreshing the page.');
        setIsModelLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    async function applyPrivacyFilter() {
      ...
      try {
        ...
        let result = await processImage(imageRef.current, modelRef.current, privacyLevel, filterType);
        
        // Then apply circular area effects if any exist
        if (circleAreas.length > 0) {
          try {
            ...
            await new Promise<void>((resolve, reject) => {
              ...
              tempImg.onerror = () => reject(new Error('Failed to load processed image'));
              tempImg.src = result;
            });
  ...
  }, [originalImage, privacyLevel, filterType, isModelLoading, circleAreas, circleBlurMode, imageDimensions]);
  ...
  const handleImageUpload = (file: File) => {
      ...
      if (modelRef.current && !isModelLoading) {
        ...
        setTimeout(async () => {
          try {
            ...
            const result = await processImage(img, modelRef.current, privacyLevel);
            setProcessedImage(result);
            setIsProcessing(false);
          } catch (err) {
    ...
    img.onerror = () => {
      setError('Failed to load the image. Please try again with a different image.');
      setIsProcessing(false);
      URL.revokeObjectURL(imageUrl);
    };
  ...
  const handleDownload = () => {
    ...
    fetch(processedImage)
      .then(res => res.blob())
      .then(blob => {
        saveAs(blob, `privacygnine-image-${new Date().toISOString().slice(0,10)}.png`);
      })
      .catch(err => {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100" style={{ fontFamily: 'Inter, sans-serif' }}>
      <div className="container px-3 sm:px-4 py-4 sm:py-8 mx-auto max-w-full sm:max-w-5xl">
        <header className="text-center mb-6 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>PrivacyGnine</h1>
          <p className="text-sm sm:text-base text-gray-600 max-w-2xl mx-auto px-2">
            Protect your privacy with browser-based image anonymization.
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden p-3 sm:p-6 mb-4 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 space-y-2 sm:space-y-0">
              <div className="flex items-center space-x-2">
                <h2 className="text-lg sm:text-xl font-semibold">PrivacyGnine</h2>
                <span className="text-xs px-2 py-1 bg-green-100 text-green-800 rounded-full">Pro</span>
              </div>
              {originalImage && (
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4 space-y-3 sm:space-y-0">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Image Preview</h2>
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center px-2 sm:px-3 py-1 sm:py-2 border border-transparent rounded-md shadow-sm text-xs sm:text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
        ...
        <footer className="mt-6 sm:mt-12 text-center text-xs sm:text-sm text-gray-500 px-2">
          <p>PrivacyGnine - All processing happens in your browser. Your images never leave your device.</p>
          <p className="mt-1">Built with React.js and TensorFlow.js</p>
          <p className="mt-2 text-xs">&copy; {new Date().getFullYear()} PrivacyGnine - <a href="#" className="text-blue-500 hover:text-blue-700">Privacy Policy</a></p>
        </footer>
      </div>
    </div>
  );
