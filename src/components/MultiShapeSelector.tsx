...
export const MultiShapeSelector: React.FC<MultiShapeSelectorProps> = ({
  ...
  const drawResizeHandles = (ctx: CanvasRenderingContext2D, handlePositions: Array<{x: number, y: number, cursor: string, handle: string}>) => {
    handlePositions.forEach(pos => {
      ...
      // Increase handle size on mobile for better touch targets
      const handleSize = isMobileDevice() ? 14 : 6;
  ...
  const getResizeHandleAt = (x: number, y: number, area: Area): string | null => {
    ...
    // Significantly larger touch target for mobile devices
  ...
  const handleTouchStart = (e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    // Create a position object using our helper function
    const rect = canvas.getBoundingClientRect();
    const scaleX = canvas.width / rect.width;
    const scaleY = canvas.height / rect.height;
    const touchPos = {
    ...
    // Create a new shape - only prevent default if we're actually creating a shape
    // This allows scrolling when not interacting with the canvas
